...
def folder_check(search_directory):
    ...
    if not os.path.exists(script_path+'OUPUT\\'):
        os.mkdir('OUPUT\\')
...
def create_output_file(matrix_file_name):
    try:
        script_path = os.getcwd()+"\\"
        output_file =  open(script_path+'OUPUT\\'+matrix_file_name+".h",'r')
        output_file.close()

    except IOError:
        print(Fore.CYAN + 'creating '+matrix_file_name+".h")
        script_path = os.getcwd()+"\\"
        output_file =  open(script_path+'OUPUT\\'+matrix_file_name+".h",'w')
...
def conversion(search_directory, matrix_file_name,display_width,display_height):
    if (folder_check(search_directory)):
        ...
        if(len(image_list)>0):
            ...
            try:
                script_path = os.getcwd()+"\\"
                # print(script_path+'OUPUT\\'+matrix_file_name+".h")
                output_file =  open(script_path+'OUPUT\\'+matrix_file_name+".h",'a')
                for frame in range(frames):
                    ...
                    output_file.write("\n")
                output_file.close()
                print(script_path+'OUPUT\\'+matrix_file_name+".h succesfully saved")
